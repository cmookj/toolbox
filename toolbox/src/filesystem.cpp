...
fs::path prepend_dir (const std::string& dir, const std::string& file_name) {
    return (
        dir.length() > 0 ? fs::path {dir} / fs::path {file_name}
                         : fs::path {file_name}
    );
}

bool file_exists (const fs::path& filepath, const std::size_t length) {
    if (length == 0)
        return fs::exists (filepath);
    else
        return (fs::exists (filepath) && fs::file_size (filepath) >= length);
}

bool file_exists (const std::string_view& filename, const std::size_t length) {
...
std::string check_duplicate (
    ...
    while (file_exists (std::string_view {base + ext})) {
        base = std::string {filename} + std::string {"_"} +
               std::to_string (++duplicated);
    }
    return base + ext;
}

// Check whether there exists the directory given in a directory
bool directory_exists (const std::string_view& name, const fs::path& dir) {
    if (!fs::exists (dir))
        throw std::runtime_error {"The path given does not exist"};

    for (const auto& p : fs::directory_iterator (dir)) {
        if (fs::is_directory (p) && p.path().filename() == name)
            return true;
    }
...
void move_item_to (const fs::path& item, const fs::path& dst) {
    if (item_movable (item, dst)) {
        try {
            if (fs::is_directory (item)) {
                create_new_directory_in (
                    fs::canonical (item).filename().string(), dst
                );
                move_directory_contents_to (
                    item, dst / fs::path {fs::canonical (item).filename()}
                );
            }
        ...
        throw std::runtime_error {"The item not movable"};
}

// Copy the filesystem item given to the path specified
void copy_item_to (const fs::path& item, const fs::path& dst) {
    if (item_movable (item, dst)) {
        try {
            if (fs::is_directory (item)) {
                create_new_directory_in (
...
void move_directory_contents_to (const fs::path& src, const fs::path& dst) {
    for (auto& p : fs::directory_iterator (src)) {
        if (!item_movable (p.path(), dst))
            throw std::runtime_error {
                "The contents of the directory not moveable"
            };
    }
...
void copy_directory_contents_to (const fs::path& src, const fs::path& dst) {
    for (auto& p : fs::directory_iterator (src)) {
        if (!item_movable (p.path(), dst))
            throw std::runtime_error {
                "The contents of the directory not moveable"
...
void remove_all_contents (const fs::path& p, const bool delete_directory) {
    if (!fs::exists (p) || !fs::is_directory (p))
        throw std::runtime_error {
            "The directory of which to remove the contents does not exist"
        };
...
void create_new_directory_in (
    ...
    if (fs::exists (dst / fs::path {name}))
        throw std::runtime_error {"The directory already exists"};
...
bool item_movable (const fs::path& item, const fs::path& dst) {
    ...
    if (fs::is_directory (dst)) { // Destination is directory
        if (fs::exists (dst)) {   // Destination exists
            if (dst.filename() ==
                "") { // The destination is the inside of the directory
                if (fs::exists (
                        fs::canonical (dst) / fs::canonical (item).filename()
                    ))
                    return false;
                else
                    return true;
            } else { // The destination is the directory itself.
                if (dst.filename() == item.filename())
                    return false;
                else
                    return true;
            }
 ...
 */
std::string to_string (const fs::path& file_path) {
...
// path.
fs::path find_file (
    const std::string_view& dir, const std::string_view& file, fs::path current
) {
...
fs::path find_directory (const std::string_view& dir, fs::path current) {
    if (!current.has_root_path()) {
        throw std::runtime_error {"Current path does not have root path"};
    }
...
bool create_symlink (const fs::path target, const fs::path link) {
    if (!fs::exists (target)) {
        log::warning (
            "No symbolic link is creates: the target '{}' does not exist.",
            target.string()
        );
        return false;
    }
    ...
    if (fs::is_symlink (link)) {
        log::info ("Pre-existing link is deleted.");
        fs::remove (link);
    }
...
bool create_directory_symlink (const fs::path target, const fs::path link) {
    if (!fs::exists (target)) {
        log::warning (
            "No symbolic link is created: the target '{}' does not exist.",
            target.string()
        );
        return false;
    }
...
fs::path follow_target_path (const fs::path& link) {
    if (is_symlink (link)) {
        log::info ("Following the symbolic link: {}", link.string());
